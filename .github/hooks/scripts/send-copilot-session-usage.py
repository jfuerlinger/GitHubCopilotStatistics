...
def detect_actor(cwd: Path) -> str | None:
    """Best-effort actor identity, since the agentStop hook input has none.

    Precedence: explicit override, GitHub Actions identity, then local Git
    config. Returns None if nothing is available; the ingestion API then
    falls back to a query parameter, a header, or "unknown".
    """
    for env_var in ("COPILOT_USAGE_ACTOR", "GITHUB_ACTOR"):
        value = os.environ.get(env_var)
        if value and value.strip():
            return value.strip()

    return git_value(cwd, "config", "user.name") or git_value(cwd, "config", "user.email")
