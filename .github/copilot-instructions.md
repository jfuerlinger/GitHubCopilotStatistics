...
- The sender script (`.github/hooks/scripts/send-copilot-session-usage.py`) must continue sanitizing credential-bearing Git remotes, accept both `sessionId` and legacy `session_id`, and report AI credits in whole credit units rather than nano-AIU.
