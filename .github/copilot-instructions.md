...
1. `.github/hooks/copilot-usage-webhook.json` registers an `agentStop` hook. It downloads the standard-library-only Python sender from the public Copilot usage Gist and executes it through `uv`. The sender reads the hook JSON from stdin, gets usage from `~/.copilot/session-store.db` or falls back to the session `events.jsonl`, adds Git metadata, and posts the resulting payload.
...
- The Gist sender must continue sanitizing credential-bearing Git remotes, accept both `sessionId` and legacy `session_id`, and report AI credits in whole credit units rather than nano-AIU.
